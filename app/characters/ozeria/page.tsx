...
export default function PR3_1Page() {
  ...
  const [beziehungenSection, setBeziehungenSection] = useState<
    Record<string, string>
  >({});
  ...
  useEffect(() => {
    ...
    const parseCharacterSections = (text: string) => {
      ...
      if (caelesteNameMatch)
        setCaelesteName(
          caelesteNameMatch[1].replace("Bild 2", "Fairy Tale").trim()
        );
      ...
      if (caelesteMatch) {
        if (typeof caelesteMatch[1] === "string")
          setCaelesteSection(
            caelesteMatch[1]
              .replaceAll(/\[Bild .\]/g, "")
              .replace("  ", " ")
      ...
      // Extract Inventar section
      const inventarMatch = text.match(/『Inventar』([\s\S]*?)(?=『|$)/);
      if (inventarMatch) {
        if (typeof inventarMatch[1] === "string")
          setInventarSection(inventarMatch[1]);
      }
    ...
    putCharacterInLocalstorage("ozeria");
    ...
    const fetchCharacterData = async () => {
      try {
        ...
        const response = await fetch("/api/fetch-doc", {
          ...
          body: JSON.stringify({
            url: "https://docs.google.com/document/d/e/2PACX-1vQInBATlqkKBwPt265yjlx6OyjdTA05446mvkEJbs6AaJ71cl985ezI_AbeBivJY7oFNb2W18ZdxvGf/pub",
            name: name,
            docname: `${name} - PR3`,
          }),
        });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {beziehungenSection && (
                <div className="border p-3 rounded">
                  ...
                  <div className="text-sm">
                    {Object.entries(beziehungenSection).map(([key, value]) => (
                      <div key={key}>
